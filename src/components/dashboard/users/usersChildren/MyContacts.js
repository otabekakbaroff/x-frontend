...
function MyContacts() {
    ...
    // globalContext
    const { setReceiver, userState } = useContext(userContext);

    useEffect(() => {
        axiosWithAuth()
            .get(`/api/connections/${userState.user}/friends-list`)
            .then(result => {
                ...
                if (!localStorage.getItem('receiver-username')) {
                    ...
                    setReceiver(result.data[0].username)
                }
    ...
    function clickHandler() {
        console.log("YOU CLICKED A CONTACT")
    }
